...
void $(Peripheral_Name)$_Init(void)
{
	...
	/* Enable GPIOA Clock */
	RCC_AHB1PeriphClockCmd($(Peripheral_Name)$Control_SetClock, ENABLE);

	/* Select GPIO Push-Pull Output Mode */
	GPIO_InitStructure.GPIO_Pin = $(Peripheral_Name)$_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_$(GPIO_Mode)$;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_$(GPIO_Speed)$MHz;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_$(GPIO_OType)$;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_$(GPIO_PuPd)$;

	GPIO_Init($(Peripheral_Name)$_PORT, &GPIO_InitStructure);
}
/* !Generate Code */
